...
Help()
{
   # Display Help
    ...
    echo "#         atm_ml_prefix   : Should contain ua, pfull at all model levels      #"
    echo "#         atm_2d_prefix   : Should contain ps, tas, pr, rlut, uas, vas, clt   #"
...
cdo -O -P ${PROCS} -gencon,r180x91 -selvar,tas "${ATM_2D_FILES[0]}" "${ATM_2D_WGHTS}"
...
atm_2d_processing() {
    ...
    for var in tas pr rlut uas vas ;
    do
        echo ""
        echo "Remapping: ${var}"
        out_file="${tmpdir}/${var}.gr2.${filename}"
        check_for_and_remove_incomplete_files "${out_file}"
        if [ ! -e "${out_file}" ];
        then
            echo "Remapped output will be saved to: ${out_file}"
            cdo -P ${PROCS} -remap,r180x91,"${ATM_2D_WGHTS}" -selvar,"${var}" "${in_file}" "${out_file}"
        else
    ...
    do
        ...
        then
            echo "Remapped output will be saved to: ${out_file}"
            cdo -P ${PROCS} -mulc,100 -remap,r180x91,"${ATM_2D_WGHTS}" -selvar,"${var}" "${in_file}" "${out_file}"
        else
            echo "  skipping rempaping"
        fi
    done
}
...
for var in tas pr rlut uas vas clt;
do
...
atm_ml_processing() {
    in_file=$1
    sister_file="${in_file/_atm_ml_1mth_mean_/_atm_2d_1mth_mean_}"
    filename=$(basename "${in_file}")
    echo ""
    echo "Operating on ${in_file}"
    ...
    then
        ...
        cdo -P ${PROCS} -remap,r180x91,"${ATM_2D_WGHTS}" -ap2pl,30000 -merge -selvar,ua,pfull "${in_file}" -selvar,ps "${sister_file}" "${ua300_out}"
    else
        echo "  skipping remapping"
    fi
    ...
    then
        ...
        cdo -P "${PROCS}" -remap,r180x91,"${ATM_2D_WGHTS}" -setmisstoc,5500 -selvar,zg -ap2pl,50000 -merge -selvar,zg "${atm_zg_file}" -merge [ -selvar,pfull "${in_file}" -selvar,ps "${sister_file}" ] "${zg500_out}"
    else
        echo "  skipping remapping"
    fi
}
...
cdo -O -P ${PROCS} -gencon,r180x91 -setctomiss,0 -selvar,ssh "${OCE_2D_FILES[0]}" "${ZOS_WGHTS}"
...
oce_2d_processing() {
    ...
    then
        ...
        cdo -P ${PROCS} -remap,r180x91,"${SI_WGHTS}" -chname,mlotst10,mlotst -selvar,mlotst10 "${in_file}" "${mlotst_out}"
    else
        echo "  skipping remapping"
    fi
    ...
    then
        ...
        cdo -P ${PROCS} -remap,r180x91,"${OCE_2D_WGHTS}" -setctomiss,0 -chname,to,tos -selvar,to "${in_file}" "${tos_out}"
    else
        echo "  skipping remapping"
    fi
